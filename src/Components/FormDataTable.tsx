...
import { SubmittedFormProps } from "../assets/formConfigs";

function FormDataTable({ SubmittedDataList }: SubmittedFormProps) {
                    ...
                    <tbody>
                        {SubmittedDataList?.map((form, index) => (
                            <tr key={index}>
                                ...
                                <td>
                                    <Link to={`/form?id=${form?.formId}`}>{form?.submitBy}</Link>
                                </td>
